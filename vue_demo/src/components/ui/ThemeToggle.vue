...
.theme-toggle {
  ...
  font-size: 16px !important; /* 固定大小 ≈ 90px × 40px */
  background: none;
  border: none;
  cursor: pointer;
  padding: 0;
  margin: 0 15px; /* 与旁边元素保持间距 */
...
<!-- 全局样式：必须无 scoped，才能响应 html.dark -->
<style>
...
.theme-toggle__container:hover .theme-toggle__sun {
  transform: translateX(calc(var(--sun-offset) + 0.187em)) !important;
}
html.dark .theme-toggle__container:hover .theme-toggle__sun {
  transform: translateX(calc(var(--toggle-width) - var(--sun-diameter) - var(--sun-offset) - 0.187em)) !important;
}
</style>