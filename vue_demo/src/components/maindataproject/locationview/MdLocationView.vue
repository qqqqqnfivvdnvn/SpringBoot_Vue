    ...
    <div class="integrated-container">
        ...
        <div v-else class="card-container">
          <el-scrollbar>
            <el-row :gutter="20" style="margin: 0; padding: 16px;">
              <el-col v-for="item in locationData.list" :key="item.id" :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
                <el-card class="location-card" shadow="hover">
                  ...
                  <div class="card-body">
                    <div class="card-item"><span class="label">ID：</span><el-link type="primary" :underline="false" @click="copyText(item.id)">{{ item.id }}</el-link></div>
                    ...
                    <div class="card-item"><span class="label">原始名称：</span>{{ item.originalName }}</div>
                    ...
                    <div class="card-item"><span class="label">城市：</span>{{ item.city }}</div>
                    <div class="card-item"><span class="label">区县：</span>{{ item.areaName }}</div>
                    <div class="card-item"><span class="label">区县编码：</span>{{ item.areaId }}</div>
                    <div class="card-item"><span class="label">经纬度：</span><span v-if="item.lngLat">{{ item.lngLat }}</span><span v-else style="color: #999">未匹配</span></div>
                    <div class="card-item"><span class="label">创建时间：</span>{{ formatDateTimeValue(item.createTime) }}</div>
                  </div>
                </el-card>
              </el-col>
            </el-row>
            <el-empty v-if="!locationData.list?.length" description="没有找到匹配的地理位置数据" />
          </el-scrollbar>
        </div>
      </div>
      ...
      <div class="fixed-pagination" v-if="locationData.list?.length">
        <div class="pagination-content">
          <el-button size="small" plain class="page-btn" :disabled="locationData.isFirstPage" @click="pageNumber > 1 && (pageNumber--, fetchData())">
            上一页
          </el-button>
          <div class="page-jumper">
            ...
            <el-input-number
              ...
              class="page-input"
            />
            <span class="page-total">页，共 {{ locationData.pages }} 页 ({{ locationData.total }} 条)</span>
          </div>
          <el-button size="small" plain class="page-btn" :disabled="locationData.isLastPage" @click="pageNumber < locationData.pages && (pageNumber++, fetchData())">
            下一页
          </el-button>
        </div>
      </div>
...
/* ==================== 6. 暗色模式样式 ==================== */
html.dark .md-location-view,
.dark .md-location-view {
