<template>
  <div class="md-location-upload-view" :style="themeStyle">
    <!-- 文件上传区域 -->
    <div class="upload-container">
      <div class="upload-card">
        <div class="upload-title">上传批次文件</div>
        ...
        <div class="template-section">
          ...
          <el-descriptions :column="1" border size="small">
            ...
            <el-descriptions-item label="处理说明">
              文件上传后将自动调用高德地图 API 获取地理位置信息，处理完成后状态变为"已处理"
            </el-descriptions-item>
          </el-descriptions>
        </div>
      </div>
  ...
  try {
    ...
    if (data.code === 200) {
      uploadResult.value = {
        success: true,
        message: '批次文件上传成功',
        details: '文件已提交到后台处理，正在调用高德地图 API 获取地理位置信息。请稍后在批次列表中查看处理状态。',
      }
      handleRemove()
    } else {
