<template>
  <div class="md-drugstore-upload-view" :style="themeStyle">
    <!-- 文件上传区域 -->
    <div class="upload-container">
      <div class="upload-card">
        <div class="upload-title">上传药店模糊匹配文件</div>
        ...
        >
          ...
          <div class="upload-hint">支持格式：.xlsx, .xls</div>
        ...
        <div class="template-section">
          ...
          <el-descriptions :column="1" border size="small">
            ...
            <el-descriptions-item label="处理说明">
              文件上传后将自动进行药店（商业公司）模糊匹配处理，根据名称和省份匹配药店数据
            </el-descriptions-item>
          </el-descriptions>
        </div>
      </div>
...
function loadThemeColor() {
  const savedColor = sessionStorage.getItem('themeColor_MainDataHome')
  if (savedColor) {
...
const submitFile = async () => {
  ...
  try {
    ...
    const {data} = await axios.post('/api/maindata/fuzzymatch/upload/drugstore', formData, {
...
.custom-upload :deep(.el-upload-dragger) {
  border: 1px dashed var(--theme-primary, #90be6d);
  border-radius: 3px;
  padding: 20px 12px;
  text-align: center;
  cursor: pointer;
...
.custom-upload :deep(.el-upload-dragger.is-dragover) {
  border-color: var(--theme-dark, #7da85a);
  background-color: rgba(144, 190, 109, 0.05);
}
