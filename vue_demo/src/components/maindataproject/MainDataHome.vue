  ...
  <div class="home-page" :style="themeStyle">
    ...
    <nav class="navbar">
      <div class="navbar-left">
        <div class="logo-area">
          ...
          <div class="brand-text">
            <span class="brand-title">主数据管理</span>
      ...
      <div class="navbar-center">
        <router-link :to="{ name: 'MainDataHome' }" class="nav-btn">
    ...
    <div class="main-content">
      ...
      <aside class="sidebar" :class="{ collapsed: isSidebarCollapsed }">
        ...
        <ul class="sidebar-menu">
          <!-- 经纬度获取 -->
          <li :class="{ 'menu-open': openMenus.location }">
            <div class="menu-item" @click="toggleMenu('location')">
              <div class="menu-item-icon">
                <font-awesome-icon :icon="['fas', 'map-marked-alt']" />
              </div>
              <span class="menu-item-text" v-if="!isSidebarCollapsed">区县经纬度</span>
              <span class="menu-arrow" v-if="!isSidebarCollapsed">
                <font-awesome-icon :icon="openMenus.location ? ['fas', 'chevron-down'] : ['fas', 'chevron-right']" size="xs" />
              </span>
            </div>
            <ul v-if="openMenus.location && !isSidebarCollapsed" class="sub-menu">
              <li @click="showBatchList">
...
function updateThemeForDarkMode() {
  const colors = document.documentElement.classList.contains('dark') ? darkThemeColors : lightThemeColors
  themeColors.value = { ...colors }
  updateElementPlusTheme(colors)
}
...
function loadThemeColor() {
  ...
  if (savedColor) {
    ...
    // 根据当前暗色模式状态决定是否使用保存的颜色
    if (document.documentElement.classList.contains('dark')) {
      // 暗色模式下使用暗色主题配置
      themeColors.value = { ...darkThemeColors }
      updateElementPlusTheme(darkThemeColors)
    } else {
      // 亮色模式下使用保存的颜色
      themeColors.value = savedThemeColors
      updateElementPlusTheme(savedThemeColors)
    }
  } else if (routeName === 'MainDataHome') {
    // 默认颜色：绿色
...
function updateElementPlusTheme(colors) {
  const root = document.documentElement
  root.style.setProperty('--el-color-primary', colors.primary)

  // 计算并设置各个色阶
  const primaryHex = colors.primary.replace('#', '')
  root.style.setProperty('--el-color-primary-light-3', adjustColor(primaryHex, 20))
  root.style.setProperty('--el-color-primary-light-5', adjustColor(primaryHex, 40))
  root.style.setProperty('--el-color-primary-light-7', adjustColor(primaryHex, 60))
  root.style.setProperty('--el-color-primary-light-9', adjustColor(primaryHex, 80))
  root.style.setProperty('--el-color-primary-dark-2', adjustColor(primaryHex, -10))

  // 更新 success 颜色（绿色系）
  root.style.setProperty('--el-color-success', colors.primary)
  root.style.setProperty('--el-color-success-light-3', adjustColor(primaryHex, 20))
  root.style.setProperty('--el-color-success-light-5', adjustColor(primaryHex, 40))
  root.style.setProperty('--el-color-success-light-7', adjustColor(primaryHex, 60))
  root.style.setProperty('--el-color-success-light-9', adjustColor(primaryHex, 80))
  root.style.setProperty('--el-color-success-dark-2', adjustColor(primaryHex, -10))
}
...
const tabs = ref([])
const currentView = ref('MdLocationBatchView')

// 组件映射对象
const componentsMap = {
  MdLocationBatchView: 'MdLocationBatchView',
  MdLocationUploadView: 'MdLocationUploadView',
  MdLocationView: 'MdLocationView'
}

const currentViewComponent = computed(() => componentsMap[currentView.value] || 'MdLocationBatchView')
...
const closeTab = (tabId) => {
  if (tabs.value.length <= 1) {
    currentView.value = 'MdLocationBatchView'
  }
...
const showBatchList = () => {
  addTab('批次列表', 'MdLocationBatchView')
  window.location.hash = '/maindata/batch'
}

const showUpload = () => {
  addTab('文件上传', 'MdLocationUploadView')
  window.location.hash = '/maindata/upload'
}

const showLocationData = () => {
  addTab('汇总数据', 'MdLocationView')
  window.location.hash = '/maindata/location'
}
...
html.dark .navbar {
  background: linear-gradient(90deg, #2d5222 0%, #3d6b2f 60%, #4f8a3f 100%);
...
html.dark .menu-label {
  color: var(--theme-text-dark, #7dc163);
}
...
.menu-item:hover {
  background: linear-gradient(135deg, #e8f4e0, #dbe8d0);
  color: var(--theme-primary);
}

html.dark .menu-item:hover {
  background: linear-gradient(135deg, #1a3a20, #1a2f15);
  color: var(--theme-text-dark, #7dc163);
}

.menu-open > .menu-item {
  background: linear-gradient(135deg, #e5f4dd, #d0e8c8);
  color: var(--theme-primary);
}

html.dark .menu-open > .menu-item {
  background: linear-gradient(135deg, #1a4025, #1a3518);
  color: var(--theme-text-dark, #7dc163);
}
...
html.dark .sub-icon {
  color: var(--theme-icon-dark, #7dc163);
}
...
html.dark .tab.active {
  background-color: #1a2a18;
  border-color: var(--theme-primary);
  border-bottom: 2px solid #1a2a18;
  color: var(--theme-primary);
}
