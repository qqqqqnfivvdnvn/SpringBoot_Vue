        ...
        <div class="content-wrapper" v-loading="loading">
          ...
          <div v-if="viewMode === 'table'" class="table-view">
            ...
            >
              ...
              <el-table-column prop="fileCount" label="文件条数" min-width="100" align="center" />
              <el-table-column prop="dataCount" label="匹配条数" min-width="100" align="center" />
          ...
          <div v-else class="card-view">
            <el-empty v-if="!batchData.list?.length" description="没有找到匹配的批次数据" :image-size="120" class="no-data-container" />
              ...
              <el-col v-for="item in batchData.list" :key="item.batchId" :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
                <el-card class="batch-card" shadow="hover">
                  ...
                  <div class="card-body">
                    ...
                    <div class="card-item"><span class="label">文件条数：</span>{{ item.fileCount }}</div>
                    <div class="card-item"><span class="label">匹配条数：</span>{{ item.dataCount }}</div>
        ...
        <div class="fixed-pagination" v-if="batchData.list?.length">
          <div class="pagination-content">
            <el-button
                ...
                plain
                class="page-btn"
                :disabled="batchData.isFirstPage"
                @click="pageNumber > 1 && (pageNumber--, fetchBatchData())"
            >
              上一页
            </el-button>
            ...
            <el-button
                ...
                plain
                class="page-btn"
                :disabled="batchData.isLastPage"
                @click="pageNumber < batchData.pages && (pageNumber++, fetchBatchData())"
            >
              下一页
            </el-button>
...
const exportingRows = ref(new Set())  // 存储正在导出的行 batchId
...
const toExcel = async (row) => {
  ...
  try {
    ...
    link.download = `恒瑞批次数据_${row.batchId}_${new Date().getTime()}.xlsx`
    ...
    exportingRows.value.delete(row.batchId)  // 移除当前行
  }
}
...
/* ==================== 6. 暗色模式样式 ==================== */
html.dark .batch-data-view,
.dark .batch-data-view {
