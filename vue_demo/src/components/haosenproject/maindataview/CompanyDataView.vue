                ...
                <el-card class="company-card" shadow="hover">
                  <template #header>
                    <div class="card-header">
                      <div class="card-title">
                        <el-link type="primary" :underline="false" @click="copyText(company.name)">
                          {{ company.name }}
                        </el-link>
                      </div>
                      <el-tag :type="getStatusType(company.status)" size="small" effect="dark">
                        {{ getStatusText(company.status) }}
                      </el-tag>
                    </div>
                  </template>
                  <div class="card-body">
                    <div class="card-item"><span class="label">dataId：</span><el-link type="primary" :underline="false" @click="copyText(company.dataId)">{{ company.dataId }}</el-link></div>
                    <div class="card-item"><span class="label">原始编码：</span><el-link type="primary" :underline="false" @click="copyText(company.dataCode)">{{ company.dataCode }}</el-link></div>
                    <div class="card-item"><span class="label">原始名称：</span><el-link type="primary" :underline="false" @click="copyText(company.originalName)">{{ company.originalName }}</el-link></div>
                    <div class="card-item"><span class="label">keyId：</span><el-link type="primary" :underline="false" @click="copyText(company.keyid)">{{ company.keyid }}</el-link></div>
                    <div class="card-item"><span class="label">省份：</span>{{ company.province }}</div>
                    <div class="card-item"><span class="label">城市：</span>{{ company.city }}</div>
                    <div class="card-item"><span class="label">区县：</span>{{ company.area }}</div>
                    <div class="card-item"><span class="label">地址：</span>{{ company.address }}</div>
                  </div>
                  <div class="card-footer">
                    <el-dropdown size="small" @command="handleCommand" class="action-dropdown">
                      ...
                      <template #dropdown>
<!--                        :disabled="!company.hsCode"-->
                        <el-dropdown-menu class="action-dropdown-menu">
                          <el-dropdown-item :command="{action: 'detail', row: company}" class="dropdown-item detail-item">
                            <el-icon class="menu-icon"><Search /></el-icon>
                            <span class="menu-text">详情查看</span>
                          </el-dropdown-item>
                          <el-dropdown-item
                              :command="{action: 'update', row: company}"

                              class="dropdown-item update-item">
                            <el-icon class="menu-icon"><EditPen /></el-icon>
                            <span class="menu-text">数据处理</span>
                          </el-dropdown-item>
                          <el-dropdown-item :command="{action: 'status', row: company}" class="dropdown-item status-item">
                            <el-icon class="menu-icon"><Tickets /></el-icon>
                            <span class="menu-text">状态管理</span>
                          </el-dropdown-item>
                          <el-dropdown-item
                              :command="{action: 'delete', row: company}"
                              divided
                              class="dropdown-item delete-item">
                            <el-icon class="menu-icon"><Delete /></el-icon>
                            <span class="menu-text">删除记录</span>
    ...
    <!-- 详情弹窗 -->
    <el-dialog v-model="showDetailModal" title="商业详情" width="500px" destroy-on-close>
      <template #title>
        <div class="custom-dialog-title">商业详情</div>
      </template>
        ...
        <div class="detail-row" v-for="(item, key) in detailFields" :key="key">
          ...
          <span>
            <template v-if="item.copy">
              <el-link type="primary" :underline="false" @click="copyText(item.value(currentCompany))">
                {{ item.value(currentCompany) }}
              </el-link>
            </template>
            <template v-else-if="key === 'status'">
              <el-tag :type="item.type(currentCompany)" size="small" effect="dark" style="color: #fff;" >
                {{ item.value(currentCompany) }}
              </el-tag>
            </template>
            <template v-else>
    ...
    >
      ...
      <div class="dialog-fixed-header">
        <div class="header-actions">
          ...
          <el-button type="primary" :loading="isSaving" :disabled="!currentUpdateCompany.hsCode" @click="saveChanges">
            {{ isSaving ? '推送中...' : '推送更新' }}
          </el-button>
      ...
      <div class="dialog-scroll-body">
        <el-form :model="currentUpdateCompany" label-width="140px">
          <el-row :gutter="30">
            <el-col :span="12">
              <el-form-item label="批次编号"><el-input v-model="currentUpdateCompany.batchCode" readonly /></el-form-item>
              <el-form-item label="dataId"><el-input v-model="currentUpdateCompany.dataId" readonly /></el-form-item>
              <el-form-item label="原始名称"><el-input v-model="currentUpdateCompany.originalName" readonly /></el-form-item>
              <el-form-item label="数据类型">
                <el-input :value="currentUpdateCompany.dataType === '1' ? '存量' : currentUpdateCompany.dataType === '2' ? '增量' : '未知'" readonly />
              </el-form-item>
              <el-form-item label="原始编码"><el-input v-model="currentUpdateCompany.dataCode" readonly /></el-form-item>
              <el-form-item label="原始省份"><el-input v-model="currentUpdateCompany.originalProvince" readonly /></el-form-item>
              <el-form-item label="原始地址"><el-input v-model="currentUpdateCompany.originalAddress" readonly /></el-form-item>
              <el-form-item label="经销商"><el-input v-model="currentUpdateCompany.companyName" readonly /></el-form-item>
              <el-form-item label="豪森上传的编码"><el-input v-model="currentUpdateCompany.haosenCode" readonly /></el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="机构类型"  :rules="[{ required: true, message: '请选择机构类型', trigger: 'change' }]">
                <el-select v-model="currentUpdateCompany.orgType" @change="handleOrgTypeChange">
                  <el-option label="医院" value="医院" />
                  <el-option label="药店" value="药店" />
                  <el-option label="商业" value="商业" />
                </el-select>
              </el-form-item>
              <el-form-item label="keyId"><el-input v-model="currentUpdateCompany.keyid" /></el-form-item>
              <el-form-item label="标准名称"><el-input v-model="currentUpdateCompany.name" /></el-form-item>
              <el-form-item label="历史名称"><el-input v-model="currentUpdateCompany.nameHistory" /></el-form-item>
              <el-form-item label="省"><el-input v-model="currentUpdateCompany.province" /></el-form-item>
              <el-form-item label="省ID"><el-input v-model="currentUpdateCompany.provinceId" /></el-form-item>
              <el-form-item label="市"><el-input v-model="currentUpdateCompany.city" /></el-form-item>
              <el-form-item label="市ID"><el-input v-model="currentUpdateCompany.cityId" /></el-form-item>
              <el-form-item label="区县"><el-input v-model="currentUpdateCompany.area" /></el-form-item>
              <el-form-item label="区县ID"><el-input v-model="currentUpdateCompany.areaId" /></el-form-item>
              <el-form-item label="地址"><el-input v-model="currentUpdateCompany.address" /></el-form-item>
              <el-form-item label="等级">
                <el-select v-model="currentUpdateCompany.level" :disabled="isFieldDisabled('level')">
              ...
              <el-form-item label="等次">
                <el-select v-model="currentUpdateCompany.grade" :disabled="isFieldDisabled('grade')">
              ...
              <el-form-item label="所有制">
                <el-select v-model="currentUpdateCompany.publicflag" :disabled="isFieldDisabled('publicflag')">
                  <el-option label="公立" value="公立" />
                  <el-option label="民营" value="民营" />
                </el-select>
              </el-form-item>
              <el-form-item label="类别"><el-input v-model="currentUpdateCompany.classify" /></el-form-item>
              <el-form-item label="总分院kid"><el-input v-model="currentUpdateCompany.generalBranchKid" :disabled="isFieldDisabled('generalBranchKid')" /></el-form-item>
              <el-form-item label="总分院名称"><el-input v-model="currentUpdateCompany.generalBranchName" :disabled="isFieldDisabled('generalBranchName')" /></el-form-item>
              <el-form-item label="是否军队医院">
                <el-select v-model="currentUpdateCompany.militaryHos" :disabled="isFieldDisabled('militaryHos')">
                  <el-option label="是" value="1" />
                  <el-option label="否" value="0" />
                </el-select>
              </el-form-item>
              <el-form-item label="登记号"><el-input v-model="currentUpdateCompany.regcode" :disabled="isFieldDisabled('regcode')" /></el-form-item>
              <el-form-item label="有效期"><el-input v-model="currentUpdateCompany.validity" :disabled="isFieldDisabled('validity')" /></el-form-item>
              <el-form-item label="诊疗科室"><el-input v-model="currentUpdateCompany.subjects" :disabled="isFieldDisabled('subjects')" /></el-form-item>
              <el-form-item label="法人代表"><el-input v-model="currentUpdateCompany.legalperson" /></el-form-item>
              <el-form-item label="统一社会信用代码"><el-input v-model="currentUpdateCompany.usci" /></el-form-item>
              <el-form-item label="经营方式">
                <el-select v-model="currentUpdateCompany.operation" :disabled="isFieldDisabled('operation')">
                  <el-option label="零售" value="零售" />
                  <el-option label="批发" value="批发" />
                  <el-option label="连锁" value="连锁" />
                </el-select>
              </el-form-item>
              <el-form-item label="经营范围"><el-input v-model="currentUpdateCompany.scope" :disabled="isFieldDisabled('scope')" /></el-form-item>
              <el-form-item label="总分店kid"><el-input v-model="currentUpdateCompany.mainBranchKid" :disabled="isFieldDisabled('mainBranchKid')" /></el-form-item>
              <el-form-item label="总分店名称"><el-input v-model="currentUpdateCompany.mainBranchName" :disabled="isFieldDisabled('mainBranchName')" /></el-form-item>
              <el-form-item label="成立日期"><el-input v-model="currentUpdateCompany.createDate" :disabled="isFieldDisabled('createDate')" /></el-form-item>
              <el-form-item label="注册资金"><el-input v-model="currentUpdateCompany.registCapi" :disabled="isFieldDisabled('registCapi')" /></el-form-item>
              <el-form-item label="企业类型"><el-input v-model="currentUpdateCompany.econKind" :disabled="isFieldDisabled('econKind')" /></el-form-item>
              <el-form-item label="登记状态"><el-input v-model="currentUpdateCompany.signStatus" :disabled="isFieldDisabled('signStatus')" /></el-form-item>
              <el-form-item label="所属行业"><el-input v-model="currentUpdateCompany.industry" :disabled="isFieldDisabled('industry')" /></el-form-item>
              <el-form-item label="登记机关"><el-input v-model="currentUpdateCompany.belong" :disabled="isFieldDisabled('belong')" /></el-form-item>
            </el-col>
          </el-row>
        </el-form>
      </div>
...
const columns = ref([
  ...
  { label: '操作', fixed: 'right' , minWidth: 120, resizable: false }
])
...
const currentUpdateCompany = ref({})
...
const getDisabledFields = computed(() => {
  const orgType = currentUpdateCompany.value.orgType
  if (orgType === '医院') {
...
// ==================== 详情弹窗字段配置 ====================
const detailFields = {
  batchCode: { label: '批次编号', value: c => c.batchCode, copy: true },
  dataId: { label: 'dataId', value: c => c.dataId, copy: true },
  dataCode: { label: '原始编码', value: c => c.dataCode, copy: true },
  originalName: { label: '原始名称', value: c => c.originalName, copy: true },
  originalProvince: { label: '原始省份', value: c => c.originalProvince },
  originalAddress: { label: '原始地址', value: c => c.originalAddress , copy: true },
  companyName: { label: '经销商', value: c => c.companyName },
  haosenCode: { label: '豪森上传的编码', value: c => c.haosenCode },
  keyid: { label: 'keyId', value: c => c.keyid, copy: true },
  dataType: { label: '数据类型', value: c => c.dataType === '1' ? '存量' : c.dataType === '2' ? '增量' : '未知' },
  hsCode: { label: '豪森清洗后编码', value: c => c.hsCode, copy: true },
  name: { label: '标准名称', value: c => c.name, copy: true },
  nameHistory: { label: '历史名称', value: c => c.nameHistory },
  province: { label: '省份', value: c => c.province },
  provinceId: { label: '省份ID', value: c => c.provinceId },
  city: { label: '市', value: c => c.city },
  cityId: { label: '市ID', value: c => c.cityId },
  area: { label: '区县', value: c => c.area },
  areaId: { label: '区县ID', value: c => c.areaId },
  address: { label: '地址', value: c => c.address },
  usci: { label: '统一社会信用代码', value: c => c.usci },
  classify: { label: '类别', value: c => c.classify },
  econKind: { label: '企业类型', value: c => c.econKind },
  signStatus: { label: '登记状态', value: c => c.signStatus },
  industry: { label: '所属行业', value: c => c.industry },
  createDate: { label: '成立日期', value: c => c.createDate },
  registCapi: { label: '注册资金', value: c => c.registCapi },
  legalperson: { label: '法人', value: c => c.legalperson },
  scope: { label: '经营范围', value: c => c.scope },
  belong: { label: '登记机关', value: c => c.belong },
  repeatId: { label: '豪森重复数据ID', value: c => c.repeatId },
  isInsert: { label: '是否新增', value: c => c.isInsert },
  field1: { label: '经纬度', value: c => c.field1 },
  field3: {
    label: '电子处方',
    value: h =>
        h.field3 === '1' ? '业务正常' :
  ...
  remarks: { label: '备注', value: c => c.remarks },
  addtime: { label: '添加日期', value: c => formatDateTime(c.addtime) },
  updatetime: { label: '更新日期', value: c => formatDateTime(c.updatetime) },
  status: {
    label: '状态',
    value: c => getStatusText(c.status),
    type: c => getStatusType(c.status)
  }
}
...
const handleJumpPage = () => {
  if (jumpPageNumber.value >= 1 && jumpPageNumber.value <= companyData.pages) {
...
const toExcel = async () => {
  ...
  try {
    ...
    link.href = url
    link.download = `豪森商业数据导出_${new Date().toLocaleDateString()}.xlsx`
    link.click()
...
const updateDetail = (row) => {
  originalCompany.value = JSON.parse(JSON.stringify(row))
  currentUpdateCompany.value = JSON.parse(JSON.stringify(row))
...
const clearDisabledFields = () => {
  ...
  disabledFieldNames.forEach(field => {
    if (field in currentUpdateCompany.value) {
      currentUpdateCompany.value[field] = '';
    }
  });
};
...
const findDaKuData = async () => {
  ...
  if (!currentUpdateCompany.value.keyid) {
  ...
  try {
    ...
    if (data.code === 200 && data.data) {
      ...
      const protectedFields = {
        batchCode: currentUpdateCompany.value.batchCode,
        dataId: currentUpdateCompany.value.dataId,
        dataType: currentUpdateCompany.value.dataType,
        dataCode: currentUpdateCompany.value.dataCode,
        originalName: currentUpdateCompany.value.originalName,
        originalProvince: currentUpdateCompany.value.originalProvince,
        originalAddress: currentUpdateCompany.value.originalAddress,
        companyName: currentUpdateCompany.value.companyName,
        appealRemark: currentUpdateCompany.value.appealRemark,
        solveRemark: currentUpdateCompany.value.solveRemark,
        orgType: currentUpdateCompany.value.orgType,
        haosenCode: currentUpdateCompany.value.haosenCode,
        hsCode: currentUpdateCompany.value.hsCode
      }

      // 更新其他字段
      Object.assign(currentUpdateCompany.value, data.data)

      // 强制恢复原始字段
      Object.assign(currentUpdateCompany.value, protectedFields)
...
const resetForm = () => {
  ElMessageBox.confirm('确定要重置所有修改吗？重置后将恢复为原始数据。', '确认重置', { type: 'warning' })
      .then(() => {
        currentUpdateCompany.value = JSON.parse(JSON.stringify(originalCompany.value))
...
const handleRepush = async () => {
  ...
  // 验证必要字段
  if (!currentUpdateCompany.value.dataId) {
...
const showDeleteDialog = (row) => {
  ElMessageBox.confirm(
    `
    <strong>商业名称：</strong>${row.name || '未知'}<br/>
...
const confirmUpdateStatus = async () => {
  ...
  try {
    // 调用API更新状态
    const payload = {
      dataId: currentCompany.value.dataId,
...
/* 暗色模式支持 */
html.dark .company-data-view,
.dark .company-data-view {
...
html.dark .company-card :deep(.el-card__header),
.dark .company-card :deep(.el-card__header) {
  background: var(--el-bg-color, #1a1a2c) !important;
  border-bottom-color: var(--el-border-color, #3a3a4a) !important;
}

html.dark .company-card :deep(.el-card__body),
.dark .company-card :deep(.el-card__body) {
  background: var(--el-bg-color, #1a1a2c) !important;
}
