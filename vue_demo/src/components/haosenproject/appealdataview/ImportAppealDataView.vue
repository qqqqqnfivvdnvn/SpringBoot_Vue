        ...
        <div class="template-section">
          ...
          <el-descriptions :column="1" border size="small">
            <el-descriptions-item label="必需字段">
              批次编号、data_id、数据类型、原始数据编码、原始数据名称、省份、原始数据地址、经销商、申诉备注、申诉解决、机构类型、keyid、医院名称、历史名称、省、省 ID、市、市 ID、区县、区县 ID、地址、等级、等次、所有制、类别、总分院 kid、总分院名称、军队医院、登记号、有效期、诊疗科室、法人代表、统一社会信用代码、经营方式、经营范围、总分店 kid、总分店名称、成立时间、注册资金、企业类型、登记状态、所属行业、登记机关
            </el-descriptions-item>
            <el-descriptions-item label="文件格式">
              .xlsx 或 .xls
            </el-descriptions-item>
            <el-descriptions-item label="处理说明">
              文件上传后将自动处理，处理完成后数据将推送至豪森
            </el-descriptions-item>
          </el-descriptions>
        </div>
      </div>
...
const submitFile = async () => {
  ...
  try {
    ...
    const {data} = await axios.post('/api/haosen/appealdata/importappealdata', formData, {
    ...
    if (data.code === 200) {
      uploadResult.value = {
        success: true,
        message: data.data.message || '申诉数据推送成功',
        details: `已成功处理${data.data.result || 0}条申诉数据`,
      }
      pendingCount.value += data.data.result || 0
      handleRemove()
    } else {
      throw new Error(data.msg || '申诉数据推送失败')
    }
  } catch (error) {
    console.error(error)
    uploadResult.value = {
...
.custom-upload :deep(.el-upload-dragger.is-dragover) {
  border-color: #a488dc;
  background-color: rgba(164, 136, 220, 0.05);
}
