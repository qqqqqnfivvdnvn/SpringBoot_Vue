  ...
  <div class="home-page" :style="themeStyle">
    ...
    <nav class="navbar">
      <div class="navbar-left">
        <div class="logo-area">
          ...
          <div class="brand-text">
            <span class="brand-title">豪森主数据</span>
      ...
      <div class="navbar-center">
        <router-link :to="{ name: 'HaoSenHome' }" class="nav-btn" @click="resetToDashboard">
    ...
    <div class="main-content">
      ...
      <aside class="sidebar" :class="{ collapsed: isSidebarCollapsed }">
        ...
        <ul class="sidebar-menu">
          <!-- 主数据 -->
          <li :class="{ 'menu-open': openMenus.mainDataManagement }">
            <div class="menu-item" @click="toggleMenu('mainDataManagement')">
              <div class="menu-item-icon">
                <font-awesome-icon :icon="['fas', 'th-list']" />
              </div>
              <span class="menu-item-text" v-if="!isSidebarCollapsed">主数据</span>
              <span class="menu-arrow" v-if="!isSidebarCollapsed">
                <font-awesome-icon :icon="openMenus.mainDataManagement ? ['fas', 'chevron-down'] : ['fas', 'chevron-right']" size="xs" />
              </span>
            </div>
            <ul v-if="openMenus.mainDataManagement && !isSidebarCollapsed" class="sub-menu">
              <li @click="showHospitalData">
                <font-awesome-icon :icon="['fas', 'hospital-alt']" class="sub-icon" />
                <span>医院主数据</span>
              </li>
              <li @click="showDrugStoreData">
                <font-awesome-icon :icon="['fas', 'store-alt']" class="sub-icon" />
                <span>药店主数据</span>
              </li>
              <li @click="companyDataView">
                <font-awesome-icon :icon="['fas', 'synagogue']" class="sub-icon" />
                <span>商业主数据</span>
              </li>
            </ul>
          </li>
          ...
          <!-- 数据清洗 -->
          <li :class="{ 'menu-open': openMenus.cleanManagement }">
            <div class="menu-item" @click="toggleMenu('cleanManagement')">
              <div class="menu-item-icon">
                <font-awesome-icon :icon="['fas', 'oil-can']" />
              </div>
              <span class="menu-item-text" v-if="!isSidebarCollapsed">数据清洗</span>
              <span class="menu-arrow" v-if="!isSidebarCollapsed">
                <font-awesome-icon :icon="openMenus.cleanManagement ? ['fas', 'chevron-down'] : ['fas', 'chevron-right']" size="xs" />
              </span>
            </div>
            <ul v-if="openMenus.cleanManagement && !isSidebarCollapsed" class="sub-menu">
              <li @click="selectCleanData">
                <font-awesome-icon :icon="['fas', 'paw']" class="sub-icon" />
                <span>清洗查看</span>
              </li>
              <li @click="importCleanData">
                <font-awesome-icon :icon="['fas', 'eraser']" class="sub-icon" />
                <span>清洗提交</span>
              </li>
            </ul>
          </li>
          ...
          <!-- 数据申诉 -->
          <li :class="{ 'menu-open': openMenus.appealManagement }">
            <div class="menu-item" @click="toggleMenu('appealManagement')">
              <div class="menu-item-icon">
                <font-awesome-icon :icon="['fas', 'flag']" />
              </div>
              <span class="menu-item-text" v-if="!isSidebarCollapsed">数据申诉</span>
              <span class="menu-arrow" v-if="!isSidebarCollapsed">
                <font-awesome-icon :icon="openMenus.appealManagement ? ['fas', 'chevron-down'] : ['fas', 'chevron-right']" size="xs" />
              </span>
            </div>
            <ul v-if="openMenus.appealManagement && !isSidebarCollapsed" class="sub-menu">
              <li @click="showAppealData">
                <font-awesome-icon :icon="['fas', 'book-reader']" class="sub-icon" />
                <span>申诉查看</span>
              </li>
              <li @click="importAppealData">
                <font-awesome-icon :icon="['fas', 'cat']" class="sub-icon" />
                <span>申诉提交</span>
              </li>
            </ul>
          </li>
          ...
          <!-- 数据更新 -->
          <li :class="{ 'menu-open': openMenus.updateManagement }">
            <div class="menu-item" @click="toggleMenu('updateManagement')">
              <div class="menu-item-icon">
                <font-awesome-icon :icon="['fas', 'file-alt']" />
              </div>
              <span class="menu-item-text" v-if="!isSidebarCollapsed">数据更新</span>
              <span class="menu-arrow" v-if="!isSidebarCollapsed">
                <font-awesome-icon :icon="openMenus.updateManagement ? ['fas', 'chevron-down'] : ['fas', 'chevron-right']" size="xs" />
              </span>
            </div>
            <ul v-if="openMenus.updateManagement && !isSidebarCollapsed" class="sub-menu">
              <li @click="importUpdateData">
                <font-awesome-icon :icon="['fas', 'snowman']" class="sub-icon" />
                <span>更新提交</span>
              </li>
            </ul>
          </li>
          ...
          <!-- 数据核查 -->
          <li :class="{ 'menu-open': openMenus.repeatDataManagement }">
            <div class="menu-item" @click="toggleMenu('repeatDataManagement')">
              <div class="menu-item-icon">
                <font-awesome-icon :icon="['fas', 'feather']" />
              </div>
              <span class="menu-item-text" v-if="!isSidebarCollapsed">数据核查</span>
              <span class="menu-arrow" v-if="!isSidebarCollapsed">
                <font-awesome-icon :icon="openMenus.repeatDataManagement ? ['fas', 'chevron-down'] : ['fas', 'chevron-right']" size="xs" />
              </span>
            </div>
            <ul v-if="openMenus.repeatDataManagement && !isSidebarCollapsed" class="sub-menu">
              <li @click="showRepeatData">
                <font-awesome-icon :icon="['fas', 'paper-plane']" class="sub-icon" />
                <span>重复数据查看</span>
              </li>
              <li @click="showExportHSData">
                <font-awesome-icon :icon="['fas', 'leaf']" class="sub-icon" />
                <span>客户确认数据</span>
              </li>
            </ul>
          </li>
...
// ThemeToggle 已移至全局 App.vue，此处无需引入
...
const themeColors = ref({ ...lightThemeColors })
...
function updateThemeForDarkMode() {
  if (document.documentElement.classList.contains('dark')) {
    themeColors.value = { ...darkThemeColors }
  } else {
    themeColors.value = { ...lightThemeColors }
  }
}
...
const showRepeatData = () => {
  addTab('重复数据查看', 'GetRepeatDataView')
  window.location.hash = '/repeatData/getRepeatData'
}


const showExportHSData = () => {
  addTab('客户确认数据', 'GetExportHSDataView')
  window.location.hash = '/repeatData/exportHSData'
}
...
.menu-item:hover {
  background: linear-gradient(135deg, #f0eaff, #e8dff8);
  color: var(--theme-primary);
}

html.dark .menu-item:hover {
  background: linear-gradient(135deg, #2a1f4a, #251940);
  color: var(--theme-text-dark, #c8b8ee);
}

.menu-open > .menu-item {
  background: linear-gradient(135deg, #ede5ff, #ddd4f8);
  color: var(--theme-primary);
}

html.dark .menu-open > .menu-item {
  background: linear-gradient(135deg, #2e2250, #26195e);
  color: var(--theme-text-dark, #c8b8ee);
}
...
html.dark .menu-arrow {
  color: var(--theme-text-dark, #6a5a8e);
}
...
html.dark .sub-menu li {
  color: var(--theme-text-dark, #a898cc);
}
...
html.dark .sub-icon {
  color: var(--theme-icon-dark, #7060a8);
}
...
html.dark .tab.active {
  ...
  color: var(--theme-text-dark, #b8a0e8);
}
