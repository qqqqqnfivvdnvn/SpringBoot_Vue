  ...
  <div class="appeal-management-view" :style="themeStyle">
    ...
    <div class="upload-container">
      <div class="upload-card">
        <div class="upload-title">上传总分院店编码</div>
        ...
        <div class="template-section">
          ...
          <el-descriptions :column="1" border size="small">
            ...
            <el-descriptions-item label="处理说明">
              文件上传后将自动根据机构类型导入到对应的编码表中，已存在的编码会自动更新
            </el-descriptions-item>
          </el-descriptions>
        </div>
      </div>
...
const submitFile = async () => {
  ...
  try {
    ...
    const {data} = await axios.post('/api/haosen/kehu/branchcode/import', formData, {
    ...
    if (data.code === 200) {
      ...
      uploadResult.value = {
        success: true,
        message: '编码导入成功',
        details: `医院 ${result.hosCount || 0} 条，药店 ${result.drugCount || 0} 条，跳过 ${result.skipCount || 0} 条`,
      }
      handleRemove()
    } else {
      throw new Error(data.msg || '编码导入失败')
    }
  } catch (error) {
    console.error(error)
    uploadResult.value = {
