<template>
  <div class="hs-medical-alliance-view">
...
const fetchData = async () => {
  ...
  try {
    ...
    const { data } = await axios.get('/api/haosen/kehu/hsmedicalalliance/getlist', { params })
    if (data.code === 200) {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const { data } = await axios.post(
        isEdit.value ? '/api/haosen/kehu/hsmedicalalliance/update' : '/api/haosen/kehu/hsmedicalalliance/insert',
        formData
      )
      if (data.code === 200) {
...
const confirmDelete = async (row) => {
  try {
    const { data } = await axios.post('/api/haosen/kehu/hsmedicalalliance/delete', { memberUnitKeyid: row.memberUnitKeyid })
    if (data.code === 200) {
...
html.dark .hs-medical-alliance-view,
.dark .hs-medical-alliance-view {
