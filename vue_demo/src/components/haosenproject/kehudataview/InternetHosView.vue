      ...
      <div class="fixed-search">
        <el-form :inline="true" :model="searchForm" class="search-form" @submit.prevent="handleSearch">
          <div class="search-conditions">
            ...
            <el-form-item label="互联网医院类别">
              <el-select v-model="searchForm.internetHosClassify" placeholder="请选择类别" clearable @clear="handleSearch" @keyup.enter="handleSearch" style="width: 150px;">
      ...
      <div class="data-content" v-loading="loading">
        ...
        <div v-if="viewMode === 'table'" class="table-container">
          ...
          >
            ...
            <el-table-column prop="internetHos" label="是否互联网医院" min-width="120" align="center">
              <template #default="{ row }">
                {{ row.internetHos }}
              </template>
            </el-table-column>
            <el-table-column prop="internetHosClassify" label="互联网医院类别" min-width="150" show-overflow-tooltip />
            ...
            <el-col v-for="item in dataList.list" :key="item.keyid" :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
              <el-card class="data-card" shadow="hover">
                ...
                <div class="card-body">
                  ...
                  <div class="card-item"><span class="label">是否互联网医院：</span>{{ item.internetHos }}</div>
                  <div class="card-item"><span class="label">互联网医院类别：</span>{{ item.internetHosClassify }}</div>
    ...
    >
      <el-form :model="formData" :rules="formRules" ref="formRef" label-width="120px">
        ...
        <el-form-item label="名称" prop="name">
          <el-input v-model="formData.name" placeholder="请输入名称（非必填）" />
        </el-form-item>
        <el-form-item label="是否互联网医院" prop="internetHos">
          <el-radio-group v-model="formData.internetHos">
            <el-radio value="是">是</el-radio>
            <el-radio value="否">否</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="互联网医院类别" prop="internetHosClassify">
          <el-select v-model="formData.internetHosClassify" placeholder="请选择互联网医院类别" style="width: 100%;" clearable>
            <el-option label="A类" value="A类" />
            <el-option label="B类" value="B类" />
            <el-option label="C类" value="C类" />
          </el-select>
...
const formRules = {
  ...
  internetHos: [{ required: false, trigger: 'blur' }]
}
...
const fetchData = async () => {
  ...
  try {
    ...
    const { data } = await axios.get('/api/haosen/kehu/internethos/getlist', { params })
    if (data.code === 200) {
...
const showAddModal = () => {
  ...
  formData.keyid = ''
  formData.name = ''
  formData.internetHos = '否'
  formData.internetHosClassify = ''
  formData.remark = ''
...
const showEditModal = (row) => {
  ...
  formData.internetHos = row.internetHos || '否'
  formData.internetHosClassify = row.internetHosClassify || ''
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const { data } = await axios.post(
        isEdit.value ? '/api/haosen/kehu/internethos/update' : '/api/haosen/kehu/internethos/insert',
        formData
      )
      if (data.code === 200) {
...
const confirmDelete = async (row) => {
  try {
    const { data } = await axios.post('/api/haosen/kehu/internethos/delete', { keyid: row.keyid })
    if (data.code === 200) {
...
@media (min-width: 2000px) and (max-width: 2600px) {
  .internet-hos-view { max-width: min(1860px, 90vw); }
}
@media (min-width: 2600px) {
  .internet-hos-view { max-width: min(2400px, 95vw); }
}

html.dark .internet-hos-view,
.dark .internet-hos-view {
