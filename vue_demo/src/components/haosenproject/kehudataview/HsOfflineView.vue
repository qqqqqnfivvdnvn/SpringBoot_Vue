  ...
  <div class="hs-offline-view">
        ...
        <div v-if="viewMode === 'table'" class="table-container">
          ...
          >
            ...
            <el-table-column prop="originalName" label="原始名称" min-width="150" show-overflow-tooltip>
              <template #default="{ row }">
                <el-link type="primary" :underline="false" @click="copyText(row.originalName)" :disabled="!row.originalName">
                  {{ row.originalName }}
                </el-link>
              </template>
            </el-table-column>
          ...
          <el-row :gutter="20" v-else>
            <el-col v-for="item in dataList.list" :key="item.dataCode" :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
              <el-card class="data-card" shadow="hover">
                ...
                <div class="card-body">
                  <div class="card-item"><span class="label">原始编码：</span><el-link type="primary" :underline="false" @click="copyText(item.dataCode)" :disabled="!item.dataCode">{{ item.dataCode }}</el-link></div>
                  <div class="card-item"><span class="label">原始名称：</span><el-link type="primary" :underline="false" @click="copyText(item.originalName)" :disabled="!item.originalName">{{ item.originalName }}</el-link></div>
                  <div class="card-item"><span class="label">标准名称：</span><el-link type="primary" :underline="false" @click="copyText(item.name)" :disabled="!item.name">{{ item.name }}</el-link></div>
    ...
    >
      <el-form :model="formData" :rules="formRules" ref="formRef" label-width="120px">
        <el-form-item label="原始编码" prop="dataCode">
          <el-input v-model="formData.dataCode" placeholder="请输入原始编码" :disabled="isEdit" />
        </el-form-item>
        <el-form-item label="原始名称" prop="originalName">
          <el-input v-model="formData.originalName" placeholder="请输入原始名称" />
        </el-form-item>
        <el-form-item label="标准名称" prop="name">
          <el-input v-model="formData.name" placeholder="请输入标准名称" />
        </el-form-item>
        <el-form-item label="豪森编码" prop="hsCode">
          <el-input v-model="formData.hsCode" placeholder="请输入豪森编码" />
        </el-form-item>
...
const formRules = {
  dataCode: [{ required: true, message: '请输入原始编码', trigger: 'blur' }],
  originalName: [{ required: false, trigger: 'blur' }],
...
const fetchData = async () => {
  ...
  try {
    ...
    const { data } = await axios.get('/api/haosen/kehu/hscode/getlist', { params })
    if (data.code === 200) {
...
const showEditModal = (row) => {
  isEdit.value = true
  formData.dataCode = row.dataCode || ''
  formData.originalName = row.originalName || ''
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const { data } = await axios.post(
        isEdit.value ? '/api/haosen/kehu/hscode/update' : '/api/haosen/kehu/hscode/insert',
        formData
      )
      if (data.code === 200) {
...
const showDeleteConfirm = (row) => {
  ElMessageBox.confirm(
    `<strong>标准名称：</strong>${row.name || '未知'}<br/><strong>原始编码：</strong>${row.dataCode}<br/><span style="color: #f56c6c;">此操作不可恢复！</span>`,
    '删除确认',
    {
...
const confirmDelete = async (row) => {
  try {
    const { data } = await axios.post('/api/haosen/kehu/hscode/delete', { dataCode: row.dataCode })
    if (data.code === 200) {
...
@media (min-width: 2000px) and (max-width: 2600px) {
  .hs-offline-view { max-width: min(1860px, 90vw); }
}

@media (min-width: 2600px) {
  .hs-offline-view { max-width: min(2400px, 95vw); }
}

html.dark .hs-offline-view,
.dark .hs-offline-view {
...
:deep(.dark .el-table--border .el-table__inner-wrapper:before),
:deep(.dark .el-table--border .el-table__inner-wrapper:after) {
  background-color: var(--el-table-border-color);
  content: "";
  position: absolute;
  z-index: calc(var(--el-table-index) + 2);
