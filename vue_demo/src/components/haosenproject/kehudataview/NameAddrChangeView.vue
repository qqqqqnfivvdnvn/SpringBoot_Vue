<template>
  <div class="name-addr-change-view">
        ...
        <div v-if="viewMode === 'table'" class="table-container">
          ...
          >
            ...
            <el-table-column prop="address" label="地址" min-width="200" show-overflow-tooltip>
              <template #default="{ row }">
                <el-link type="primary" :underline="false" @click="copyText(row.address)" :disabled="!row.address">
                  {{ row.address }}
                </el-link>
              </template>
            </el-table-column>
            <el-table-column prop="field" label="字段类型" min-width="120" align="center" />
            ...
            <el-col v-for="item in dataList.list" :key="item.keyid" :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
              <el-card class="data-card" shadow="hover">
                ...
                <div class="card-body">
                  ...
                  <div class="card-item"><span class="label">字段类型：</span>{{ item.field }}</div>
    ...
    >
      <el-form :model="formData" :rules="formRules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="地址" prop="address">
          <el-input v-model="formData.address" placeholder="请输入地址（非必填）" />
        </el-form-item>
        <el-form-item label="字段类型" prop="field">
          <el-select v-model="formData.field" placeholder="请选择字段类型" style="width: 100%;">
            <el-option label="name" value="name" />
            <el-option label="address" value="address" />
            <el-option label="name_address" value="name_address" />
          </el-select>
        </el-form-item>
...
const fetchData = async () => {
  ...
  try {
    ...
    const { data } = await axios.get('/api/haosen/kehu/nameaddr/getlist', { params })
    if (data.code === 200) {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const { data } = await axios.post(
        isEdit.value ? '/api/haosen/kehu/nameaddr/update' : '/api/haosen/kehu/nameaddr/insert',
        formData
      )
      if (data.code === 200) {
...
const confirmDelete = async (row) => {
  try {
    const { data } = await axios.post('/api/haosen/kehu/nameaddr/delete', { keyid: row.keyid })
    if (data.code === 200) {
...
html.dark .name-addr-change-view,
.dark .name-addr-change-view {
