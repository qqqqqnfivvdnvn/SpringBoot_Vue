<template>
  <div class="hs-medical-community-view">
        ...
        <el-form :inline="true" :model="searchForm" class="search-form" @submit.prevent="handleSearch">
          <div class="search-conditions">
            <el-form-item label="成员单位">
              <el-input v-model="searchForm.memberUnit" placeholder="请输入成员单位" clearable @clear="handleSearch" @keyup.enter="handleSearch" />
            </el-form-item>
            <el-form-item label="牵头单位">
              <el-input v-model="searchForm.leadingUnit" placeholder="请输入牵头单位" clearable @clear="handleSearch" @keyup.enter="handleSearch" />
            </el-form-item>
            <el-form-item label="省份">
              <el-input v-model="searchForm.province" placeholder="请输入省份" clearable @clear="handleSearch" @keyup.enter="handleSearch" />
            </el-form-item>
            <el-form-item label="牵头单位豪森">
              <el-input v-model="searchForm.hsCode" placeholder="请输入牵头单位豪森" clearable @clear="handleSearch" @keyup.enter="handleSearch" />
            </el-form-item>
            <el-form-item label="备注">
              <el-input v-model="searchForm.remark" placeholder="请输入备注" clearable @clear="handleSearch" @keyup.enter="handleSearch" />
            </el-form-item>
      ...
      <div class="data-content" v-loading="loading">
        ...
        <div v-if="viewMode === 'table'" class="table-container">
          <el-table
            ...
            resizable
          >
            <el-table-column prop="memberUnitKeyid" label="成员单位 keyid" min-width="150" show-overflow-tooltip>
              <template #default="{ row }">
                <el-link type="primary" :underline="false" @click="copyText(row.memberUnitKeyid)" :disabled="!row.memberUnitKeyid">
                  {{ row.memberUnitKeyid }}
                </el-link>
              </template>
            </el-table-column>
            <el-table-column prop="memberUnit" label="成员单位" min-width="150" show-overflow-tooltip>
              <template #default="{ row }">
                <el-link type="primary" :underline="false" @click="copyText(row.memberUnit)" :disabled="!row.memberUnit">
                  {{ row.memberUnit }}
                </el-link>
              </template>
            </el-table-column>
            <el-table-column prop="leadingUnitKeyid" label="牵头单位 keyid" min-width="150" show-overflow-tooltip>
              <template #default="{ row }">
                <el-link type="primary" :underline="false" @click="copyText(row.leadingUnitKeyid)" :disabled="!row.leadingUnitKeyid">
                  {{ row.leadingUnitKeyid }}
                </el-link>
              </template>
            </el-table-column>
            <el-table-column prop="leadingUnit" label="牵头单位" min-width="150" show-overflow-tooltip>
              <template #default="{ row }">
                <el-link type="primary" :underline="false" @click="copyText(row.leadingUnit)" :disabled="!row.leadingUnit">
                  {{ row.leadingUnit }}
                </el-link>
              </template>
            </el-table-column>
            <el-table-column prop="cooperationType" label="类型" min-width="100" show-overflow-tooltip />
            <el-table-column prop="province" label="省份" min-width="80" show-overflow-tooltip />
            <el-table-column prop="hsCode" label="牵头单位豪森" min-width="120" show-overflow-tooltip>
              <template #default="{ row }">
        ...
        <div v-else class="card-view">
          ...
          <el-row :gutter="20" v-else>
            <el-col v-for="item in dataList.list" :key="item.memberUnitKeyid" :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
              <el-card class="data-card" shadow="hover">
                <template #header>
                  <div class="card-header">
                    <div class="card-title">
                      <el-link type="primary" :underline="false" @click="copyText(item.memberUnit)">
                        {{ item.memberUnit }}
                      </el-link>
                    </div>
                  </div>
                ...
                <div class="card-body">
                  <div class="card-item"><span class="label">成员单位 keyid：</span><el-link type="primary" :underline="false" @click="copyText(item.memberUnitKeyid)">{{ item.memberUnitKeyid }}</el-link></div>
                  <div class="card-item"><span class="label">成员单位：</span><el-link type="primary" :underline="false" @click="copyText(item.memberUnit)">{{ item.memberUnit }}</el-link></div>
                  <div class="card-item"><span class="label">牵头单位 keyid：</span><el-link type="primary" :underline="false" @click="copyText(item.leadingUnitKeyid)">{{ item.leadingUnitKeyid }}</el-link></div>
                  <div class="card-item"><span class="label">牵头单位：</span><el-link type="primary" :underline="false" @click="copyText(item.leadingUnit)">{{ item.leadingUnit }}</el-link></div>
                  <div class="card-item"><span class="label">类型：</span>{{ item.cooperationType }}</div>
                  <div class="card-item"><span class="label">省份：</span>{{ item.province }}</div>
                  <div class="card-item"><span class="label">牵头单位豪森：</span><el-link type="primary" :underline="false" @click="copyText(item.hsCode)" :disabled="!item.hsCode">{{ item.hsCode }}</el-link></div>
                  <div class="card-item"><span class="label">备注：</span>{{ item.remark }}</div>
    ...
    >
      <el-form :model="formData" :rules="formRules" ref="formRef" label-width="140px">
        <el-form-item label="成员单位 keyid" prop="memberUnitKeyid">
          <el-input v-model="formData.memberUnitKeyid" placeholder="请输入成员单位 keyid" />
        </el-form-item>
        <el-form-item label="成员单位" prop="memberUnit">
          <el-input v-model="formData.memberUnit" placeholder="请输入成员单位" />
        </el-form-item>
        <el-form-item label="牵头单位 keyid" prop="leadingUnitKeyid">
          <el-input v-model="formData.leadingUnitKeyid" placeholder="请输入牵头单位 keyid" />
        </el-form-item>
        <el-form-item label="牵头单位" prop="leadingUnit">
          <el-input v-model="formData.leadingUnit" placeholder="请输入牵头单位" />
        </el-form-item>
        <el-form-item label="类型" prop="cooperationType">
          <el-input v-model="formData.cooperationType" placeholder="请输入类型" />
        </el-form-item>
        <el-form-item label="省份" prop="province">
          <el-input v-model="formData.province" placeholder="请输入省份" />
        </el-form-item>
        <el-form-item label="牵头单位豪森" prop="hsCode">
          <el-input v-model="formData.hsCode" placeholder="请输入牵头单位豪森" />
        </el-form-item>
...
const formRules = {
  memberUnitKeyid: [{ required: true, message: '请输入成员单位 keyid', trigger: 'blur' }],
  memberUnit: [{ required: false, trigger: 'blur' }],
  leadingUnit: [{ required: false, trigger: 'blur' }],
  cooperationType: [{ required: false, trigger: 'blur' }],
  province: [{ required: false, trigger: 'blur' }],
  hsCode: [{ required: false, trigger: 'blur' }],
...
const fetchData = async () => {
  ...
  try {
    ...
    const { data } = await axios.get('/api/haosen/kehu/hsmedicalcommunity/getlist', { params })
    if (data.code === 200) {
...
const showAddModal = () => {
  isEdit.value = false
  formData.memberUnit = ''
  formData.memberUnitKeyid = ''
  formData.leadingUnit = ''
  formData.leadingUnitKeyid = ''
...
const showEditModal = (row) => {
  isEdit.value = true
  formData.memberUnit = row.memberUnit || ''
  formData.memberUnitKeyid = row.memberUnitKeyid
  formData.leadingUnit = row.leadingUnit || ''
  formData.leadingUnitKeyid = row.leadingUnitKeyid || ''
  formData.cooperationType = row.cooperationType || ''
  formData.province = row.province || ''
  formData.hsCode = row.hsCode || ''
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const { data } = await axios.post(
        isEdit.value ? '/api/haosen/kehu/hsmedicalcommunity/update' : '/api/haosen/kehu/hsmedicalcommunity/insert',
        formData
      )
      if (data.code === 200) {
...
const showDeleteConfirm = (row) => {
  ElMessageBox.confirm(
    `<strong>成员单位：</strong>${row.memberUnit || '未知'}<br/><strong>成员单位 keyid：</strong>${row.memberUnitKeyid}<br/><strong>牵头单位：</strong>${row.leadingUnit || '未知'}<br/><span style="color: #f56c6c;">此操作不可恢复！</span>`,
    '删除确认',
    {
...
const confirmDelete = async (row) => {
  try {
    const { data } = await axios.post('/api/haosen/kehu/hsmedicalcommunity/delete', { memberUnitKeyid: row.memberUnitKeyid })
    if (data.code === 200) {
...
html.dark .hs-medical-community-view,
.dark .hs-medical-community-view {
  background: var(--el-bg-color, #1a1a2c);
}

@media (min-width: 2000px) and (max-width: 2600px) {
  .hs-medical-community-view { max-width: min(1860px, 90vw); }
}
@media (min-width: 2600px) {
  .hs-medical-community-view { max-width: min(2400px, 95vw); }
}
...
html.dark :deep(.el-table--border .el-table__inner-wrapper:before),
html.dark :deep(.el-table--border .el-table__inner-wrapper:after),
html.dark :deep(.el-table--border:before),
