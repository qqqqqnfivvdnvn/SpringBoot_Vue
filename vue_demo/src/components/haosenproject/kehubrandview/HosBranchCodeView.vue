<template>
  <div class="hos-branch-code-view">
...
const fetchData = async () => {
  ...
  try {
    ...
    const { data } = await axios.get('/api/haosen/kehu/hosbranchcode/getlist', { params })
    if (data.code === 200) {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const { data } = await axios.post(
        isEdit.value ? '/api/haosen/kehu/hosbranchcode/update' : '/api/haosen/kehu/hosbranchcode/insert',
        formData
      )
      if (data.code === 200) {
...
const confirmDelete = async (row) => {
  try {
    const { data } = await axios.post('/api/haosen/kehu/hosbranchcode/delete', { keyid: row.keyid })
    if (data.code === 200) {
...
html.dark .hos-branch-code-view,
.dark .hos-branch-code-view {
