<template>
  <div class="drug-branch-code-view">
      ...
      <div class="fixed-search">
        <el-form :inline="true" :model="searchForm" class="search-form" @submit.prevent="handleSearch">
          <div class="search-conditions">
            ...
            <el-form-item label="总分店编码">
              <el-input v-model="searchForm.hsCode" placeholder="请输入总分店编码" clearable @clear="handleSearch" @keyup.enter="handleSearch" />
            </el-form-item>
            <el-form-item label="备注">
              <el-input v-model="searchForm.remarks" placeholder="请输入备注" clearable @clear="handleSearch" @keyup.enter="handleSearch" />
            </el-form-item>
      ...
      <div class="data-content" v-loading="loading">
        ...
        <div v-if="viewMode === 'table'" class="table-container">
          ...
          >
            ...
            <el-table-column prop="hsCode" label="总分店编码" min-width="150" show-overflow-tooltip>
              <template #default="{ row }">
            ...
            <el-col v-for="item in dataList.list" :key="item.keyid" :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
              <el-card class="data-card" shadow="hover">
                ...
                <div class="card-body">
                  ...
                  <div class="card-item"><span class="label">名称：</span><el-link type="primary" :underline="false" @click="copyText(item.name)">{{ item.name }}</el-link></div>
                  <div class="card-item"><span class="label">总分店编码：</span><el-link type="primary" :underline="false" @click="copyText(item.hsCode)" :disabled="!item.hsCode">{{ item.hsCode }}</el-link></div>
    ...
    >
      <el-form :model="formData" :rules="formRules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="总分店编码" prop="hsCode">
          <el-input v-model="formData.hsCode" placeholder="请输入总分店编码" />
        </el-form-item>
...
const fetchData = async () => {
  ...
  try {
    ...
    const { data } = await axios.get('/api/haosen/kehu/drugbranchcode/getlist', { params })
    if (data.code === 200) {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const { data } = await axios.post(
        isEdit.value ? '/api/haosen/kehu/drugbranchcode/update' : '/api/haosen/kehu/drugbranchcode/insert',
        formData
      )
      if (data.code === 200) {
...
const confirmDelete = async (row) => {
  try {
    const { data } = await axios.post('/api/haosen/kehu/drugbranchcode/delete', { keyid: row.keyid })
    if (data.code === 200) {
...
/* 暗色模式支持 */
html.dark .drug-branch-code-view,
.dark .drug-branch-code-view {
...
html.dark .data-card :deep(.el-card__header),
.dark .data-card :deep(.el-card__header) {
  background: var(--el-bg-color, #1a1a2c) !important;
  border-bottom-color: var(--el-border-color, #3a3a4a) !important;
}

html.dark .data-card :deep(.el-card__body),
