  ...
  <div class="appeal-management-view" :style="themeStyle">
    ...
    <div class="upload-container">
      <div class="upload-card">
        <div class="upload-title">上传更新数据</div>
...
const submitFile = async () => {
  ...
  try {
    ...
    const {data} = await axios.post('/api/haosen/updateData/importUpdateData', formData, {
    ...
    if (data.code === 200) {
      uploadResult.value = {
        success: true,
        message: data.data.message || '更新数据推送成功',
        details: `已成功处理${data.data.result || 0}条更新数据`,
      }
      pendingCount.value += data.data.result || 0
      handleRemove()
    } else {
      throw new Error(data.msg || '更新数据推送失败')
    }
  } catch (error) {
    console.error(error)
    uploadResult.value = {
