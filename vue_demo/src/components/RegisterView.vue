  ...
  <div class="register-page">
    ...
    <div class="welcome-section">
      <div class="welcome-content">
        ...
        <h1>主数据管理系统</h1>
        <p class="welcome-subtitle">加入我们，开始高效的数据管理之旅</p>
      </div>
    </div>
    ...
    <div class="form-section">
      <form @submit.prevent="handleSubmit" class="form-wrapper">
        ...
        <button type="submit" :disabled="isLoading" class="submit-btn">
          ...
          <font-awesome-icon v-else :icon="['fas', 'user-plus']" style="margin-right:6px" />
          {{ isLoading ? '注册中...' : '注册' }}
        </button>
...
const handleSubmit = async () => {
  ...
  if (!trimmedUsername || !email.value || !password.value) {
    showToastMessage('用户名、邮箱和密码不能为空', 'warning')
    return
  }
  ...
  try {
    ...
    if (response.data.code === 200) {
      showToastMessage(response.data.msg || '注册成功！')
      ...
      showToastMessage(response.data.msg || '注册失败', 'error')
    }
  } catch (error) {
